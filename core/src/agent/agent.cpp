...
model_response_shared_ptr agent::generate_text(const context_shared_ptr& ctx)
{
	while (true)
	{
		...
		auto resp{ api->generate_text(impl->model_config, ctx, impl->mcp_manager ? impl->mcp_manager->get_tools() : std::vector<tool_info>{}) };
		...
		if (resp->finish_reason == "tool_calls")
		{
			...
			for (const auto& tool_call : assistant_message->get_tool_calls_ref())
			{
				try
				{
					auto tool_result{ impl->mcp_manager->call_tool(tool_call.tool_name, tool_call.arguments) };
