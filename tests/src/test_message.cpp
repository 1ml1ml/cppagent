...
TEST_CASE("message tool role 构造", "[message]")
{
  auto msg{ std::make_shared<message>(message::role::tool, "tool result"sv) };
  REQUIRE(msg->get_role() == message::role::tool);
  REQUIRE(msg->get_content() == "tool result"sv);
}
...
TEST_CASE("message role 转换字符串", "[message]")
{
  ...
  REQUIRE(std::string_view(message::role_to_string(message::role::tool)) == "tool"sv);
...
TEST_CASE("message 修改 role", "[message]")
{
  ...
  msg->set_role(message::role::tool);
  REQUIRE(msg->get_role() == message::role::tool);
}
...
TEST_CASE("message attachment 批量添加", "[message]")
{
  ...
  message::attachment att1{};
  att1.name = "a.txt";
  att1.mime_type = "text/plain";

  message::attachment att2{};
  att2.name = "b.png";
